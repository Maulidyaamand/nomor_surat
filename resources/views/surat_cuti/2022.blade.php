...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Cuti 2022</h4>
  ...
  <table class="table">
    ...
    <tbody>
        @foreach($surat_cuti2022 as $sc22)
    <tr>
        <td>{{ $sc22->nomor_urut }}</td>
        <td>{{ $sc22->nomor_surat }}</td>
        <td>{{ $sc22->isi_ringkasan }}</td>
        <td>{{ $sc22->alamat }}</td>
        <td>{{ $sc22->tanggal }}</td>
        <td>{{ $sc22->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
