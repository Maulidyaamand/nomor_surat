...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Cuti 2023</h4>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/suratcuti2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($surat_cuti2023 as $sc23)
    <tr>
        <td>{{ $sc23->nomor_urut }}</td>
        <td>{{ $sc23->nomor_surat }}</td>
        <td>{{ $sc23->isi_ringkasan }}</td>
        <td>{{ $sc23->alamat }}</td>
        <td>{{ $sc23->tanggal }}</td>
        <td>{{ $sc23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
