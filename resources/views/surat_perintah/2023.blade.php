...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Perintah 2023</h4>
    <div class="input-group w-25">
        <h6 style="margin-right:10px">Periode:</h6>
          ...
          <select class="form-control" onchange="window.location.href=this.options[this.selectedIndex].value;">
              <option value="suratperintah2021" {{ $selectedYear == '2021' ? 'selected' : '' }}>2021</option>
              <option value="suratperintah2022" {{ $selectedYear == '2022' ? 'selected' : '' }}>2022</option>
              <option value="suratperintah2023" {{ $selectedYear == '2023' ? 'selected' : '' }}>2023</option>
          </select>
    </div>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/suratperintah2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($surat_perintah2023 as $sp23)
    <tr>
        <td>{{ $sp23->nomor_urut }}</td>
        <td>{{ $sp23->nomor_surat }}</td>
        <td>{{ $sp23->nama_pegawai }}</td>
        <td>{{ $sp23->isi_ringkasan }}</td>
        <td>{{ $sp23->alamat }}</td>
        <td>{{ $sp23->tanggal }}</td>
        <td>{{ $sp23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
