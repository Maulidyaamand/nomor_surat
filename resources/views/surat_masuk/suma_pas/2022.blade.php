...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Masuk PAS 2022</h4>
...
<tbody>
  @foreach($suma_pas2022 as $smpas22)
  <tr>
      <td>{{ $smpas22->nomor_urut }}</td>
      <td>{{ $smpas22->nomor_surat }}</td>
      <td>{{ $smpas22->pengirim }}</td>
      <td>{{ $smpas22->tanggal }}</td>
      <td>{{ $smpas22->isi_ringkasan }}</td>
  </tr>
  @endforeach
</tbody>
</table>
