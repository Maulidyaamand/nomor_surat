...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Masuk PLH/PLT 2023</h4>
...
<tbody>
  @foreach($suma_plh2023 as $smplh23)
  <tr>
      <td>{{ $smplh23->nomor_urut }}</td>
      <td>{{ $smplh23->nomor_surat }}</td>
      <td>{{ $smplh23->pengirim }}</td>
      <td>{{ $smplh23->tanggal }}</td>
      <td>{{ $smplh23->isi_ringkasan }}</td>
  </tr>
  @endforeach
</tbody>
</table>
