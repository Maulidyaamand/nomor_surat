...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Masuk Umum 2021</h4>
...
<tbody>
  @foreach($suma_umum2021 as $smu21)
  <tr>
    <td>{{ $smu21->nomor_urut }}</td>
    <td>{{ $smu21->nomor_surat }}</td>
    <td>{{ $smu21->pengirim }}</td>
    <td>{{ $smu21->tanggal }}</td>
    <td>{{ $smu21->isi_ringkasan }}</td>
  </tr>
  @endforeach
</tbody>
</table>
