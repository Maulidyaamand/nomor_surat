...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Masuk Imigrasi 2023</h4>
    <div class="input-group w-25">
        <h6 style="margin-right:10px">Periode:</h6>
          ...
          <select class="form-control" onchange="window.location.href=this.options[this.selectedIndex].value;">
              <option value="sumaimig2021" {{ $selectedYear == '2021' ? 'selected' : '' }}>2021</option>
              <option value="sumaimig2022" {{ $selectedYear == '2022' ? 'selected' : '' }}>2022</option>
              <option value="sumaimig2023" {{ $selectedYear == '2023' ? 'selected' : '' }}>2023</option>
          </select>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sumaimig2023" method="POST">
      @csrf
...
<tbody>
  @foreach($suma_imig2023 as $smi23)
  <tr>
      <td>{{ $smi23->nomor_urut }}</td>
      <td>{{ $smi23->nomor_surat }}</td>
      <td>{{ $smi23->pengirim }}</td>
      <td>{{ $smi23->tanggal }}</td>
      <td>{{ $smi23->isi_ringkasan }}</td>
  </tr>
  @endforeach
</tbody>
</table>
