...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat SK KA Kanwil 2022</h4>
  ...
  <table class="table">
    ...
    <tbody>
        @foreach($surat_kakanwil2022 as $sk22)
    <tr>
        <td>{{ $sk22->nomor_urut }}</td>
        <td>{{ $sk22->nomor_surat }}</td>
        <td>{{ $sk22->isi_ringkasan }}</td>
        <td>{{ $sk22->alamat }}</td>
        <td>{{ $sk22->tanggal }}</td>
        <td>{{ $sk22->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
