...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat SK KA Kanwil 2021</h4>
  ...
  <table class="table">
    ...
    <tbody>
        @foreach($surat_kakanwil2021 as $sk21)
    <tr>
        <td>{{ $sk21->nomor_urut }}</td>
        <td>{{ $sk21->nomor_surat }}</td>
        <td>{{ $sk21->isi_ringkasan }}</td>
        <td>{{ $sk21->alamat }}</td>
        <td>{{ $sk21->tanggal }}</td>
        <td>{{ $sk21->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
