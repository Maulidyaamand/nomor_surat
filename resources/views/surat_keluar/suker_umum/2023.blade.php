...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar Umum 2023</h4>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sukerumum2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($suker_umum2023 as $sku23)
    <tr>
        <td>{{ $sku23->nomor_urut }}</td>
        <td>{{ $sku23->nomor_surat }}</td>
        <td>{{ $sku23->isi_ringkasan }}</td>
        <td>{{ $sku23->alamat }}</td>
        <td>{{ $sku23->tanggal }}</td>
        <td>{{ $sku23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
