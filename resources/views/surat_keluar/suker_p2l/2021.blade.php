...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar P2L 2021</h4>
  ...
  <table class="table">
    ...
    <tbody>
        @foreach($suker_p2l2021 as $skp21)
    <tr>
        <td>{{ $skp21->nomor_urut }}</td>
        <td>{{ $skp21->nomor_surat }}</td>
        <td>{{ $skp21->isi_ringkasan }}</td>
        <td>{{ $skp21->alamat }}</td>
        <td>{{ $skp21->tanggal }}</td>
        <td>{{ $skp21->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
