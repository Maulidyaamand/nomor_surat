...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar P2L 2023</h4>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sukerp2l2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($suker_p2l2023 as $skp23)
    <tr>
        <td>{{ $skp23->nomor_urut }}</td>
        <td>{{ $skp23->nomor_surat }}</td>
        <td>{{ $skp23->isi_ringkasan }}</td>
        <td>{{ $skp23->alamat }}</td>
        <td>{{ $skp23->tanggal }}</td>
        <td>{{ $skp23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
