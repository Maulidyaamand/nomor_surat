...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar PLH/PLT 2023</h4>
    <div class="input-group w-25">
        <h6 style="margin-right:10px">Periode:</h6>
          ...
          <select class="form-control" onchange="window.location.href=this.options[this.selectedIndex].value;">
              <option value="sukerplh2021" {{ $selectedYear == '2021' ? 'selected' : '' }}>2021</option>
              <option value="sukerplh2022" {{ $selectedYear == '2022' ? 'selected' : '' }}>2022</option>
              <option value="sukerplh2023" {{ $selectedYear == '2023' ? 'selected' : '' }}>2023</option>
          </select>
    </div>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sukerplh2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($suker_plh2023 as $skplh23)
    <tr>
        <td>{{ $skplh23->nomor_urut }}</td>
        <td>{{ $skplh23->nomor_surat }}</td>
        <td>{{ $skplh23->isi_ringkasan }}</td>
        <td>{{ $skplh23->alamat }}</td>
        <td>{{ $skplh23->tanggal }}</td>
        <td>{{ $skplh23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
