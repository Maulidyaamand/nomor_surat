...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar PAS 2023</h4>
    <div class="input-group w-25">
        <h6 style="margin-right:10px">Periode:</h6>
          ...
          <select class="form-control" onchange="window.location.href=this.options[this.selectedIndex].value;">
              <option value="sukerpas2021" {{ $selectedYear == '2021' ? 'selected' : '' }}>2021</option>
              <option value="sukerpas2022" {{ $selectedYear == '2022' ? 'selected' : '' }}>2022</option>
              <option value="sukerpas2023" {{ $selectedYear == '2023' ? 'selected' : '' }}>2023</option>
          </select>
    </div>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sukerpas2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($suker_pas2023 as $skpas23)
    <tr>
        <td>{{ $skpas23->nomor_urut }}</td>
        <td>{{ $skpas23->nomor_surat }}</td>
        <td>{{ $skpas23->isi_ringkasan }}</td>
        <td>{{ $skpas23->alamat }}</td>
        <td>{{ $skpas23->tanggal }}</td>
        <td>{{ $skpas23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
