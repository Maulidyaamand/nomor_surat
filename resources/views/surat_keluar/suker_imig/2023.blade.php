...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar Imigrasi 2023</h4>
...
<div class="modal-content">
  ...
  <div class="modal-body">
    <form action="/sukerimig2023" method="POST">
      @csrf
...
<table class="table">
    ...
    <tbody>
        @foreach($suker_imig2023 as $ski23)
    <tr>
        <td>{{ $ski23->nomor_urut }}</td>
        <td>{{ $ski23->nomor_surat }}</td>
        <td>{{ $ski23->isi_ringkasan }}</td>
        <td>{{ $ski23->alamat }}</td>
        <td>{{ $ski23->tanggal }}</td>
        <td>{{ $ski23->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
