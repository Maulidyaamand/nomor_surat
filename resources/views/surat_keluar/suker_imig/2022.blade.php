...
@section('content')
    ...
    <h4 style="margin-top:-10px; font-weight:bold; color: #A09F9F;">Surat Keluar Imigrasi 2022</h4>
  ...
  <table class="table">
    ...
    <tbody>
        @foreach($suker_imig2022 as $ski22)
    <tr>
        <td>{{ $ski22->nomor_urut }}</td>
        <td>{{ $ski22->nomor_surat }}</td>
        <td>{{ $ski22->isi_ringkasan }}</td>
        <td>{{ $ski22->alamat }}</td>
        <td>{{ $ski22->tanggal }}</td>
        <td>{{ $ski22->keterangan }}</td>
    </tr>
        @endforeach
    </tbody>
</table>
